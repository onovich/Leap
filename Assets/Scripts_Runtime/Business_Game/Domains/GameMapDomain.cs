...
namespace Leap {
    ...
    public static class GameMapDomain {
        ...
        public static void Terrain_SetAll(GameBusinessContext ctx, MapEntity map, Vector2Int[] posArr) {
            var tile = map.tileBase_terrain;
            for (int i = 0; i < posArr.Length; i++) {
                var pos = posArr[i];
                Terrain_Set(ctx, map, pos, tile);
            }
        }

        public static void Terrain_Set(GameBusinessContext ctx, MapEntity map, Vector2Int pos, TileBase tile) {
            map.Terrain_Set(pos, tile);
        }
